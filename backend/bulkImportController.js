...
exports.importData = async (req, res) => {
    ...
    try {
        ...
        // Preparar inserção - usar colunas do CSV que existem na tabela, exceto auto-geradas
        const csvColumns = Object.keys(records[0])
            .map(col => col.trim().toLowerCase())
            .filter(col => tableInfo.columns.includes(col) && !autoGeneratedCols.includes(col));
        ...
        for (const layerNum of sortedLayerNums) {
            ...
            for (const { file, tableName } of layerFiles) {
                try {
                    ...
                    // Normalizar nomes de colunas do CSV e excluir auto-geradas (id, uuid, timestamps)
                    const autoGenerated = tableInfo.autoGeneratedColumns || [];
                    const csvColumns = Object.keys(records[0])
                        .map(col => ETL.normalizeColumnName(col))
                        .filter(col => tableInfo.columns.includes(col) && !autoGenerated.includes(col));
