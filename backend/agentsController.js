    ...
    try {
        ...
        const newAgent = await executeTransaction(async (client) => {
            ...
            const agentResult = await client.query(`
                INSERT INTO chatbots (name, description, category, class, specialization_level, status)
                VALUES ($1, $2, $3, $4, $5, $6)
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updatedAgent = await executeTransaction(async (client) => {
            ...
            const agentResult = await client.query(`
                UPDATE chatbots SET
                    name = $1, description = $2, category = $3, class = $4, specialization_level = $5, status = $6, updated_at = CURRENT_TIMESTAMP
                WHERE id = $7
                RETURNING *
            `, [identity.name, identity.description, identity.category, identity.class, identity.specializationLevel, identity.status, id]);
