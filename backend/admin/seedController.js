...
const seedCampaigns = async (req, res) => {
    try {
        console.log('🌱 Iniciando seed de dados de campanhas via API...');

        // 1. Garantir Clientes
        const clientIds = [];
        for (const clientData of CLIENTS_DATA) {
            let result = await db.query('SELECT id FROM clients WHERE name = $1', [clientData.name]);
        ...
        await db.query('DELETE FROM campaigns WHERE client_id = ANY($1)', [clientIds]);
        ...
        res.json({ success: true, message: `Seed concluído! ${campaignsCreated} campanhas criadas com métricas.` });
    } catch (error) {
