-- Migration to add permissions and extended user profile fields
...
-- 2. Create Role Permissions Table (Many-to-Many)
...
-- 6. Assign Permissions to Roles (Example)
-- Admin gets all permissions
INSERT INTO role_permissions (role_id, permission_id)
SELECT (SELECT id FROM roles WHERE name = 'Admin'), id FROM permissions
ON CONFLICT DO NOTHING;

-- User gets view permissions and manage leads/campaigns
INSERT INTO role_permissions (role_id, permission_id)
SELECT (SELECT id FROM roles WHERE name = 'User'), id FROM permissions 
WHERE name IN ('view_dashboard', 'manage_campaigns', 'manage_leads', 'view_reports')
ON CONFLICT DO NOTHING;
