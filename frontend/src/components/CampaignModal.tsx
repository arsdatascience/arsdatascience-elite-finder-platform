...
export const CampaignModal: React.FC<CampaignModalProps> = ({ campaign, isOpen, onClose, onSave, mode }) => {
    ...
    const handleScheduleMeeting = async () => {
        if (!scheduleDate || !scheduleTime || !campaign) return;
        setIsScheduling(true);
        try {
            const dateTime = new Date(`${scheduleDate}T${scheduleTime}`);

            await apiClient.social.createPost({
                content: `Reunião sobre Campanha: ${campaign.name} (${scheduleType})`,
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-gray-800">
                        {mode === 'create' ? 'Nova Campanha' : 'Detalhes da Campanha'}
                    </h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                                    <Layers size={16} className="text-gray-500" /> Plataforma
                                </label>
                                <select
                                    {...register('platform')}
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                                    <DollarSign size={16} className="text-green-500" /> Investimento
                                </label>
                                <input
                                    {...register('spend')}
                                    className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                                    <BarChart2 size={16} className="text-purple-500" /> Receita
                                </label>
                                <input
                                    {...register('revenue')}
                                    className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none transition-all"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                                    <Eye size={16} className="text-orange-500" /> Impressões
                                </label>
                                <input
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                                    <MousePointer size={16} className="text-indigo-500" /> Cliques
                                </label>
                                <input
                                    {...register('clicks')}
                                    className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none transition-all"
