...
export const SocialCalendar: React.FC<SocialCalendarProps> = ({
    ...
    const { data: holidays } = useQuery({
        queryKey: ['holidays'],
        queryFn: apiClient.social.getHolidays,
        staleTime: Infinity
    });
    ...
    const generateSuggestion = async () => {
        ...
        try {
            const result = await apiClient.ai.generateMarketingContent({
                type: 'post',
                platform: 'instagram',
                topic: `Post comemorativo para ${suggestionModal.holiday.name}`,
                tone: 'Engajador e Profissional'
            });

            const newPost = {
                client_id: selectedClient === 'all' ? 1 : Number(selectedClient),
                content: `${result.headlines[0]}\n\n${result.body}\n\n${result.hashtags ? result.hashtags.join(' ') : ''}`,
                platform: 'instagram',
                status: 'draft',
                scheduled_date: suggestionModal.date.toISOString()
            };

            await apiClient.social.createPost(newPost);
            queryClient.invalidateQueries({ queryKey: ['socialPosts'] });
            setSuggestionModal(null);
            alert('Rascunho criado com sucesso! Verifique o calendário.');
