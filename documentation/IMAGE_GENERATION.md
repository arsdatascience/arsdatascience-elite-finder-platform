...
### 1. Dependências
Certifique-se de instalar as novas dependências no backend:
```bash
cd backend
...
Adicione as seguintes chaves ao seu arquivo `.env` no backend:
```env
OPENAI_API_KEY=sk-...
...
Para habilitar o rastreamento de custos e provedores, execute o script de migração:
```bash
cd backend
node migrate_analytics.js
```
*Nota: Certifique-se de que as variáveis de conexão com o banco (DATABASE_URL) estejam corretas no .env.*

## Modelos Suportados
- **Replicate**: Flux Schnell, SDXL Lightning, Z-Image Turbo
- **Hugging Face**: Stable Diffusion XL (Gratuito)
- **OpenAI**: DALL-E 3 Standard, DALL-E 3 HD
- **Google**: Gemini 2.5 Flash Image, Gemini Pro Image
